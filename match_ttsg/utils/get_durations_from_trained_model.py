...
from match_ttsg.cli import get_device
...
def save_durations_to_folder(
    attn: torch.Tensor, x_length: int, y_length: int, filepath: str, output_folder: Path 
):
...
def compute_durations(data_loader: torch.utils.data.DataLoader, model: nn.Module, device: torch.device, output_folder):
    ...
    Args:
        ...
        model (nn.Module): match_ttsgTTS model
...
def main():
    ...
    if args.output_folder is not None:
        ...
        output_folder = Path(cfg["motion_folder"]).parent / "durations"
    ...
    model = MatchTTSG.load_from_checkpoint(args.checkpoint_path, map_location=device)
...
if __name__ == "__main__":
    ...
    # Example usage:
    # python python match_ttsg/utils/get_durations_from_trained_model.py -i ljspeech.yaml -c pretrained_model
